...
from .helper import add_transaction, get_transaction_history_fnc,get_best_seller_func
...
@api_router.get("/statistical/best-seller")
async def get_best_seller(current_user= Depends(get_current_active_user)):
    best_seller_list = await get_best_seller_func(current_user.id)

    return responseModel(data=best_seller_list)